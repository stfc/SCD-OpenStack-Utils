...
def main():
    ...
    for result in results:
        ...
        datastring += ",Memory_MB_Seconds=" + str(
            result["Memory_MB"] * result["VM_Seconds"]
        )
        ...
        datastring += ",Root_GB_Seconds=" + str(
            result["Root_GB"] * result["VM_Seconds"]
        )
        datastring += ",Root_GBs=" + str(result["Root_GB"] * result["VMs"])
        datastring += ",Ephemeral_GB_Seconds=" + str(
            result["Ephemeral_GB"] * result["VM_Seconds"]
        )
        datastring += ",Ephemeral_GBs=" + str(result["Ephemeral_GB"] * result["VMs"])
        if int(accountinglib.ifnull(result["GPU_Num"], 0)) > 0:
            datastring += ",GPU_Seconds=" + str(
                float(result["GPU_Num"]) * float(result["VM_Seconds"])
            )
            datastring += ",GPUs=" + str(
                float(result["GPU_Num"]) * float(result["VMs"])
            )
            logger.info(result)
            datastring += ",COST=" + str(
                float(result["GPU_Num"])
                * float(result["VM_Seconds"])
                * float(result["Per_Unit_Cost"])
                / float(3600)
            )
        else:
            datastring += ",COST=" + str(
