...
def test_query_list_no_device(instance):
    ...
    """
    mock_device_list = []
    with patch("utils.query_device.QueryDevice.query_device"):
...
def test_query_list_one_device(instance):
    ...
    """
    mock_device_list = [""]
    with patch("utils.query_device.QueryDevice.query_device") as mock_query_device:
        res = instance.query_list(mock_device_list)
...
def test_query_device(instance, mock_device, dict_to_device):
    ...
    """
    with patch("utils.query_device.NetboxGetId.get_id") as mock_get_id:
