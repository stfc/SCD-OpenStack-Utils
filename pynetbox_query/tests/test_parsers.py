...
def test_arg_parser(mock_argparse, mock_parent_parser, instance):
    ...
    expected_subparser = mock_argparse.ArgumentParser.return_value.add_subparsers.return_value
...
def test_parent_parser(mock_argparse, instance):
    ...
    mock_parent_parser.add_argument.assert_any_call("file_path", help="Your file path to csv files.")
    mock_parent_parser.add_argument.assert_any_call("--delimiter", help="The separator in the text file.")
    mock_parent_parser.add_argument.assert_any_call("--sheet-name", help="The sheet in the Excel Workbook to read from.")
